      ...
      function createCourseCards() {
        ...
        document.querySelectorAll('.course-card').forEach(card => {
          ...
          card.querySelector('.buy-btn').addEventListener('click', function(e) {
            e.stopPropagation();
            alert('جاري تحويلك لصفحة الشراء...');
          });
        });
      }
      ...
      function showCourseDetails(courseId, category) {
        ...
        courseDetails.innerHTML = `
          ...
          <div class="details-actions">
            <button class="details-btn primary-btn">
    ...
    // تفعيل الوضع الداكن
// تفعيل الوضع الداكن - الإصدار المصحح
const themeSwitch = document.querySelector('.switch input');

// الحصول على الثيم الحالي من localStorage أو استخدام الوضع الفاتح كافتراضي
const currentTheme = localStorage.getItem('theme') || 'light';

// تطبيق الثيم الحالي
document.documentElement.setAttribute('data-theme', currentTheme);

// إذا كان الثيم الحالي داكن، نضع علامة على الزر
...
themeSwitch.addEventListener('change', function() {
  if (this.checked) {
    // إذا تم تفعيل الزر (ON) -> الوضع الداكن
    document.documentElement.setAttribute('data-theme', 'dark');
    localStorage.setItem('theme', 'dark');
  } else {
    // إذا تم إلغاء تفعيل الزر (OFF) -> الوضع الفاتح
